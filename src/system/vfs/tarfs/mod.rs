...
impl VFSImplementation for TarFS {
    fn initialize(&mut self) -> VFSResult<()> {
        let mut probable_initramfs: Option<&&limine::file::File> = None;
        ...
        if let Some(resp) = MODULE_REQUESTS.get_response() {
            for module in resp.modules() {
                if module.path().to_str().unwrap().eq("/boot/initramfs.tar") {
                    probable_initramfs = Some(module);
                    break;
                }
            }
        }

        if probable_initramfs.is_none() {
            log::warn!(
                "tarfs: no initramfs module found, skipping initialization..."
            );
            return Ok(());
        }

        if let Some(file) = probable_initramfs {
